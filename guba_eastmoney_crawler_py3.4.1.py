...
def get_post_vol(content):
    if len(content)==1:
        ...
        t1=datetime.datetime.strptime(content[1][0],'%Y-%m-%d %H:%M:%S')
...
def get_max_id(path):
    ...
    return content_list[-1][1],content_list[-1][0]
...
class Async_infi_Spider():
    def __init__(self, idcode,datetime_date, concurrency,max_concurrency=4000,num_per=500):
        ...
        self.concurrency_temp_step=concurrency
    ...
    def fetch(self,url,idcode):
        try:
            ...
            with open(self.log_saving_path_today,'a', newline='',encoding='utf-8') as f:
                f.write(idcode)
                f.write(e)
            print(e)
        ...
        while 1:
            ...
            if self.return_count==0:
                ...
                if len(self.content):
                    ...
                    print('获取发帖速度参数和最后发帖时间...',self.post_vol,self.last_post_datetime)
                    '''判断是否完成当天任务'''
                ...
                if datetime.datetime.now()-self.last_post_datetime>datetime.timedelta(hours=1):
                    self.concurrency=self.max_concurrency
                    print('并发速度为最大限速 %s'%self.concurrency)
                elif datetime.datetime.now()-self.last_post_datetime<=datetime.timedelta(hours=1):
                    if self.post_vol==0:
                        ...
                        multi_para=0.003*seconds-0.8
                        self.concurrency=max(int(multi_para*self.post_vol*60),self.concurrency_temp_step)
...
def main(start_id,requests_amount,requests_per_time):
    print('初始化')
    ...
    print('启动爬虫')
    sever_spider=Async_infi_Spider(start_code,datetime_date,requests_amount,requests_per_time)
    sever_spider._run()
if __name__ == '__main__':
    main(428106962,800,500)
